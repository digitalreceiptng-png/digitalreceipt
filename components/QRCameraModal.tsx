...
export default function QRCameraModal({ onScan, onClose }: Props) {
  ...
  const modal = (
    <div className="fixed inset-0 bg-black flex flex-col" style={{ zIndex: 9999 }}>
      ...
      {/* Video always in DOM so videoRef is non-null when the stream is attached */}
      <div className={`relative bg-black overflow-hidden ${ready ? 'flex-1' : 'hidden'}`}>
      ...
      {/* Overlay states (shown instead of camera when not ready) */}
      {!ready && (
        <div className="flex-1 flex flex-col items-center justify-center gap-5 p-6 text-center">
          {requesting ? (
            ...
            <>
              ...
              <button onClick={startCamera} className="px-6 py-3 bg-white text-black text-sm font-semibold rounded-xl hover:bg-white/90 active:scale-95 transition-all">
                Try again
              </button>
            </>
          ) : error ? (
            ...
            <>
              ...
              <button onClick={startCamera} className="px-8 py-3.5 bg-white text-black text-sm font-bold rounded-xl hover:bg-white/90 active:scale-95 transition-all">
                Enable Camera
              </button>
            </>
          )}
