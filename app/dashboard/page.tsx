  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="font-heading text-2xl text-[#0f1f13]">Welcome back, {firstName}</h1>
          <p className="text-sm text-[#4a6b55] mt-1">
            {profile?.issuer_type === 'business' ? profile?.business_name ?? profile?.full_name : profile?.full_name}
          </p>
        </div>
        <Link
          href="/dashboard/receipts/new"
          className="flex items-center gap-2 bg-[#1a6b2f] text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-[#155a27] transition-colors shrink-0"
        >
          <PlusCircle size={16} />
          New Receipt
        </Link>
      ...
      {/* Usage card */}
      <div className="bg-white rounded-xl border border-[#e0ede5] p-5">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-sm font-medium text-[#0f1f13]">Receipts this month</p>
            <p className="text-2xl font-bold text-[#0f1f13] mt-0.5">
              {used} <span className="text-base font-normal text-[#4a6b55]">of {limit}</span>
            </p>
          </div>
          {atLimit ? (
            ...
            <span className="text-xs bg-[#f4faf6] text-[#4a6b55] border border-[#e0ede5] px-2.5 py-1 rounded-full">
        ...
        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-500 ${atLimit ? 'bg-amber-500' : 'bg-[#2ecc5f]'}`}
        ...
        {atLimit && (
          <div className="mt-4 p-3 bg-amber-50 border border-amber-100 rounded-lg">
            <p className="text-sm text-amber-800">
              You&apos;ve used all {limit} of your free receipts this month. Need more?
            </p>
            <Link
      ...
      <div className="bg-white rounded-xl border border-[#e0ede5] overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="font-medium text-[#0f1f13]">Recent Receipts</h2>
          <Link href="/dashboard/receipts" className="text-sm text-[#1a6b2f] hover:underline">
            View all
          </Link>
        </div>
        ...
        {!recentReceipts?.length ? (
          <div className="py-12 text-center">
            ...
            <p className="text-sm text-[#4a6b55]">No receipts yet.</p>
            <Link
              href="/dashboard/receipts/new"
