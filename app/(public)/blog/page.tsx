...
export default async function BlogPage() {
  ...
  const merged = [
    ...
    ...staticPosts.map(p => ({ ...p, isDb: false })),
  ]
    ...
    <div className="bg-white">
      ...
      <section className="py-12 sm:py-16 px-4">
        <div className="max-w-4xl mx-auto">
          {/* Featured post */}
          <div className="mb-10">
            <Link href={`/blog/${featured.slug}`} className="group block bg-white rounded-2xl border border-border overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="p-6 sm:p-8">
                <div className="flex items-center gap-3 mb-3">
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${CATEGORY_COLORS[featured.category] ?? 'bg-surface text-ink-muted'}`}>{featured.category}</span>
                  <span className="flex items-center gap-1 text-xs text-ink-dim"><Calendar size={11} />{formatDate(featured.date)}</span>
                  <span className="flex items-center gap-1 text-xs text-ink-dim"><Clock size={11} />{featured.readTime}</span>
                </div>
                <h2 className="font-heading text-2xl sm:text-3xl text-ink mb-2 group-hover:text-forest transition-colors" style={{ textWrap: 'balance' }}>{featured.title}</h2>
                <p className="text-sm text-ink-muted leading-relaxed">{featured.excerpt}</p>
