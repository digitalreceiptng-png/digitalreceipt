    ...
    if (channel === 'sms' || channel === 'both') {
      ...
      if (buyerPhone && ownerUserId) {
        try {
          ...
          const dueMonth = dueDate.toLocaleDateString('en-NG', { month: 'long', year: 'numeric' })
          await sendTermiiSms(normalized, `Reminder: Your payment of ₦${Number(inst.amount).toLocaleString('en-NG', { minimumFractionDigits: 2 })} to ${sellerName} for ${dueMonth} is due${daysText}. View receipt: ${receiptUrl}`)
          didSend = true
