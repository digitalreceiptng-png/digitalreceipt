...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string; itemId: string }> }) {
  ...
  await db.from('receipt_item_edit_otps').insert({
    user_id: userId, item_id: itemId, new_description: description, code_hash: hashOtp(code), expires_at,
  })
  ...
  if (email) {
    try {
      ...
      await resend.emails.send({
        ...
        to: email,
        subject: `Confirm receipt item edit: ${code}`,
        html: `
          <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;background:#fff;">
            ...
            <p style="font-size:14px;color:#4a5568;margin:0 0 24px 0;">Hi ${name}, use the code below to change a receipt item's description to "<strong>${description}</strong>".</p>
            <div style="background:#f5fbf5;border:1px solid #c8e6c8;border-radius:10px;padding:20px 24px;text-align:center;margin-bottom:24px;">
              <p style="font-size:36px;font-weight:700;letter-spacing:10px;color:#0d6b1e;margin:0;font-family:monospace;">${code}</p>
            </div>
  ...
  if (phone) {
    try {
      await sendTermiiSms(
        ...
        `Your DigitalReceipt.ng code to confirm a receipt item edit is: ${code}. Valid for 10 minutes. Do not share this code.`
      )
      smsSent = true
    } catch (err) {
