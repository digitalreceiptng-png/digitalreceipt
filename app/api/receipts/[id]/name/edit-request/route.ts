...
// required before an already-generated receipt's buyer name can change.
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!buyerName) return NextResponse.json({ error: 'Enter a name.' }, { status: 400 })
  ...
  const { data: receipt } = await db
    .from('receipts')
    .select('id, user_id, buyer_name')
    .eq('id', id)
    .single()
  if (!receipt || receipt.user_id !== userId) {
    return NextResponse.json({ error: 'Receipt not found.' }, { status: 404 })
  }
  if ((receipt.buyer_name ?? '') === buyerName) {
    return NextResponse.json({ error: 'Enter a different name.' }, { status: 400 })
  }
  ...
  await db.from('receipt_name_edit_otps').delete().eq('user_id', userId).eq('receipt_id', id)
  const { error: insertErr } = await db.from('receipt_name_edit_otps').insert({
    user_id: userId,
    receipt_id: id,
    new_buyer_name: buyerName,
  ...
  if (insertErr) {
    console.error('[name edit-request] failed to save pending edit:', insertErr)
  ...
  if (email && emailCode) {
    try {
      ...
      await resend.emails.send({
        ...
        to: email,
        subject: `Confirm receipt name change: ${emailCode}`,
        html: `
          <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;background:#fff;">
            ...
            <h1 style="font-size:22px;color:#1a2e1a;margin:0 0 8px 0;font-weight:700;">Confirm receipt name change</h1>
            <p style="font-size:14px;color:#4a5568;margin:0 0 24px 0;">Hi ${name}, use the code below to change this receipt's buyer name to "<strong>${buyerName}</strong>". A second code was sent by SMS — you'll need both.</p>
      ...
      console.error('[name edit-request] email failed:', err)
    }
  }
  ...
  if (phone && phoneCode) {
    try {
      await sendTermiiSms(
        ...
        `Your DigitalReceipt.ng code to confirm a receipt name change is: ${phoneCode}. A second code was emailed — you'll need both. Valid for 10 minutes. Do not share this code.`
      )
      smsSent = true
    } catch (err) {
      console.error('[name edit-request] SMS failed:', err)
    }
  }
  ...
  if (email && !emailSent) await db.from('receipt_name_edit_otps').update({ email_code_hash: null }).eq('user_id', userId).eq('receipt_id', id)
  if (phone && !smsSent) await db.from('receipt_name_edit_otps').update({ phone_code_hash: null }).eq('user_id', userId).eq('receipt_id', id)
