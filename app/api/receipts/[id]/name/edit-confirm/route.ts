...
// POST — verify both codes, then apply the pending buyer name change for real.
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (new Date(otp.expires_at) < new Date()) {
    await db.from('receipt_name_edit_otps').delete().eq('id', otp.id)
  ...
  if (!emailOk || !phoneOk) {
    await db.from('receipt_name_edit_otps').update({ attempts: otp.attempts + 1 }).eq('id', otp.id)
  ...
  const { data: updated, error } = await db
    .from('receipts')
    .update({ buyer_name: otp.new_buyer_name })
    .eq('id', id)
    .eq('user_id', userId)
    .select('buyer_name')
    .single()

  if (error || !updated) return NextResponse.json({ error: 'Receipt not found.' }, { status: 404 })

  await db.from('receipt_name_edit_otps').delete().eq('id', otp.id)

  return NextResponse.json({ ok: true, buyerName: updated.buyer_name })
}
